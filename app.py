...
from bulma import BulmaFormStyle


app = App(__name__)
app.config.db.uri = "postgres://max:max@localhost:5432/bloggy"
...
async def one(pid):
    ...
    if session.auth:
        form = await Comment.form(onvalidation=_validate_comment, formstyle=BulmaFormStyle)
        if form.accepted:
...
async def new_post():
    form = await Post.form(formstyle=BulmaFormStyle)
    if form.accepted:
